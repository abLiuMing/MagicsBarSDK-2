...
#include <cmath>
#include "fmStaticAssert.h"

#define FM_VEC_CLAMP(n, low, high) do{\
if(n < low) n = low;\
...
namespace FM
{
	...
	public:
		...
		Vec() { fmStaticAssert(DIM > 0 && DIM <= 4);}
		Vec(const Vec& v) { VecAlgorithmHelp<Vec<Type, DIM>, DIM>::_assign(*this, v); }
		Vec(Type x)
		{
			fmStaticAssert(DIM == 1); //DIM should be 1
			m_data[0] = x;
		}

		Vec(Type x, Type y)
		{
			fmStaticAssert(DIM == 2); //DIM should be 2
			m_data[0] = x;
			m_data[1] = y;
		}
		...
		{
			fmStaticAssert(DIM == 3); //DIM should be 3
			m_data[0] = x;
			m_data[1] = y;
			m_data[2] = z;
		}
		...
		{
			fmStaticAssert(DIM == 4); //DIM should be 4
			m_data[0] = x;
			m_data[1] = y;
			m_data[2] = z;
			m_data[3] = w;
		...
		inline Type& x() { fmStaticAssert(DIM >= 1); return m_data[0]; }
		inline Type& y() { fmStaticAssert(DIM >= 2); return m_data[1]; }
		inline Type& z() { fmStaticAssert(DIM >= 3); return m_data[2]; }
		inline Type& w() { fmStaticAssert(DIM >= 4); return m_data[3]; }
		inline Type& r() { fmStaticAssert(DIM >= 1); return m_data[0]; }
		inline Type& g() { fmStaticAssert(DIM >= 2); return m_data[1]; }
		inline Type& b() { fmStaticAssert(DIM >= 3); return m_data[2]; }
		inline Type& a() { fmStaticAssert(DIM >= 4); return m_data[3]; }

		inline const Type& x() const { fmStaticAssert(DIM >= 1); return m_data[0]; }
		inline const Type& y() const { fmStaticAssert(DIM >= 2); return m_data[1]; }
		inline const Type& z() const { fmStaticAssert(DIM >= 3); return m_data[2]; }
		inline const Type& w() const { fmStaticAssert(DIM >= 4); return m_data[3]; }
		inline const Type& r() const { fmStaticAssert(DIM >= 1); return m_data[0]; }
		inline const Type& g() const { fmStaticAssert(DIM >= 2); return m_data[1]; }
		inline const Type& b() const { fmStaticAssert(DIM >= 3); return m_data[2]; }
		inline const Type& a() const { fmStaticAssert(DIM >= 4); return m_data[3]; }
