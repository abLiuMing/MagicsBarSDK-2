...
#ifndef _FMSAHDERFUNCTIONS_H_
#define _FMSAHDERFUNCTIONS_H_
...
namespace FM
{
	...
	public:
		...
		typedef struct UniformData
		{
			...
			UniformData(const char* name, UNIFORM_TYPE type) :
				uniformType(type){ strncpy(uniformName, name, FM_UNIFORM_MAX_LEN); }
			...
			char uniformName[FM_UNIFORM_MAX_LEN];
		...
		void clear();
		void assignUniforms(FMImageHandlerInterface* handler, GLuint programID);
	...
	protected:
		inline GLint _getUniform(GLuint programId, const char* name)
		{
			...
			FM_LOG_CODE(
				if(uniform < 0)
					FM_LOG_ERROR("uniform name %s does not exist!\n", name);
			);
			return uniform;
		}
