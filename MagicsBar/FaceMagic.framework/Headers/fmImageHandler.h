...
#ifndef _FMIMAGEHANDLER_H_
#define _FMIMAGEHANDLER_H_
...
namespace FM
{
	...
	{
	public:
		FMImageHandlerInterface();
		virtual ~FMImageHandlerInterface();
		...
		{	
			return FMGlobalConfig::sVertexDataCommon;
		};
		...
		const FMSizei& getOutputFBOSize() const { return m_dstImageSize; }
	...
	class FMImageHandler : public FMImageHandlerInterface
	{
	private:
		explicit FMImageHandler(const FMImageHandler&) {}
	public:
		FMImageHandler();
		virtual ~FMImageHandler();

		bool initWithRawBufferData(const void* data, GLint w, GLint h, FMBufferFormat format, bool bEnableReversion = true);
		bool updateData(const void* data, int w, int h, FMBufferFormat format);

		bool initWithTexture(GLuint textureID, GLint w, GLint h, FMBufferFormat format, bool bEnableReversion = false);

		bool getOutputBufferData(void* data, FMBufferFormat format);
		size_t getOutputBufferLen(size_t channel);
		size_t getOutputBufferBytesPerRow(size_t channel);

		void setAsTarget();
		void addImageFilter(FMImageFilterInterfaceAbstract* proc);
		void popImageFilter();
		void clearImageFilters(bool bDelMem = true);
		size_t getFilterNum() { return m_vecFilters.size(); }
		FMImageFilterInterfaceAbstract* getFilterByIndex(GLuint index)
		{ return index >= m_vecFilters.size() ? nullptr : m_vecFilters[index]; }
		int getFilterIndexByAddr(const void* addr);
		bool insertFilterAtIndex(FMImageFilterInterfaceAbstract* proc, GLuint index);
		bool deleteFilterByAddr(const void* addr, bool bDelMem = true);
		bool deleteFilterByIndex(GLuint index, bool bDelMem = true);
		bool replaceFilterAtIndex(FMImageFilterInterfaceAbstract* proc, GLuint index, bool bDelMem = true);
		bool swapFilterByIndex(GLuint left, GLuint right);
		void peekFilters(std::vector<FMImageFilterInterfaceAbstract*>* vTrans);
		std::vector<FMImageFilterInterfaceAbstract*>& peekFilters() { return m_vecFilters; }
		...
		//使用未加入的滤镜进行单独处理。
		bool processingWithFilter(FMImageFilterInterfaceAbstract* proc);
...
#ifdef _FM_USE_ES_API_3_0_

		const void* mapOutputBuffer(FMBufferFormat fmt);
	...
	protected:
		...
		std::vector<FMImageFilterInterfaceAbstract*> m_vecFilters;
